...
4. **Combinar todo** para obtener la salida `u`.
    - `u = P + I + D;` (cada término ya incluye su ganancia)
5. **Mandar la salida al actuador**
    - Ej: `ledcWrite(pinPWM, salida);` (en ESP32, con LEDC; en Arduino Uno sería `analogWrite`)
    - Ej: controlar un servo, motor, etc.
6. **Repetir** el ciclo en `loop()`.

## Ejemplo de código PID sencillo en Arduino

Queremos controlar, por ejemplo, la **velocidad** de un motor con PWM en el **ESP32** (mismas funciones LEDC del tema de Actuadores). Supongamos que tenemos:

- Un sensor analógico que mide algo entre 0 y 4095 (`analogRead`, ADC de 12 bits del ESP32).
- Un pin PWM para el motor, por ejemplo el 12 (con `ledcAttachChannel`/`ledcWrite`).
...
// Variables del PID
float setpoint = 2000;  // valor deseado (ej. 2000 de 0-4095)
...
double dt = 0;
const int pinSensor = 34;  // GPIO 34: pin ADC del ESP32 (solo entrada)
const int pinPWM = 12;

void setup() {
  /* PWM con LEDC (Tema Actuadores): pin 12, 1 kHz, 8 bits (0-255), canal 0 */
  ledcAttachChannel(pinPWM, 1000, 8, 0);
  Serial.begin(115200);

  medida = analogRead(pinSensor);
  // Inicializar el reloj AQUI evita que el primer dt sea enorme
  // (si no, la derivada da un golpe inicial: "derivative kick")
...
void loop() {
  ...
  dt = double(tiempo_actual - tiempo_anterior) / 1000.0; // ms -> s
  ...
  integral = integral + error * dt;
  // Anti-windup: limitar la acumulación de la integral
  ...
  // 5) Salida total (cada término ya incluye su ganancia)
  float u = P + I + D;
  ...
  // Pequeño delay para no saturar el puerto serie (quitarlo para mejor reacción)
  delay(10);
}

```
<!-- PID 2D demo: carro en la cancha (coordenadas de cámara) -->
...
<script>
  ...
  // Dinámica simplificada del móvil: x'' = k_gain * u - b * x'
  const b = 0.1;
  const k_plate = 5.0;
  ...
  function initPlot() {
    ...
    const layout = {
      title: 'PID 2D sobre error de posición (carro visto por la cámara cenital)',
